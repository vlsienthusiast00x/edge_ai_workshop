...
It is important to declare the weights and biases arrays in the `weights.h` as `static const` beacause this tells the program to store the weights and biases in the flash memory instead of SRAM, which also reduces the memory occupation. You can also declare the input array in the main c program as `static const`.
